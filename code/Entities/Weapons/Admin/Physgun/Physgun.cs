...
public partial class PhysGun : WeaponBase
{
	...
	protected PhysicsBodyType OriginalPhysicsType;
	...
	{
		if ( Owner is not MainPawn owner ) return;
	...
	private void TryUnfreezeAll( Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir )
	{
		...
		if ( unfrozen )
		{
			var freezeEffect = Particles.Create( "particles/weapon/physgun/physgun_freeze.vpcf" );
	...
	private void GrabEnd()
	{
		if ( heldBody.IsValid() )
		{
			...
			heldBody.BodyType = OriginalPhysicsType;
	...
	private void GrabMove( Vector3 startPos, Vector3 dir, Rotation rot, bool snapAngles )
	{
		...
		if ( GrabbedEntity is MainPawn player )
		{
			...
			UpdateGrabbedPlayer( To.Single( player ), player.EyePosition );
	...
	[ClientRpc]
	void UpdateGrabbedPlayer(Vector3 pos)
	{
		Camera.Position = pos;
	}
