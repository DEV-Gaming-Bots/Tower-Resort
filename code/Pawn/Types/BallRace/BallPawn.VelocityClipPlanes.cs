...
public struct BallVelocityClipPlanes : IDisposable
{
	...
	public BallVelocityClipPlanes( Vector3 originalVelocity, int max = 10 )
	{
		Max = max;
	...
	public bool TryAdd( Vector3 normal, Vector3 planeVelocity, ref Vector3 velocity )
	{
		...
		if ( Count == 1 )
		{
			...
			BumpVelocity = ClipVelocity( BumpVelocity, planeVelocity, normal, 0.035f );
	...
	bool TryClip( ref Vector3 velocity, Vector3 planeVelocity )
	{
		...
		{
			velocity = ClipVelocity( BumpVelocity, planeVelocity, Planes[i], 0.035f );
	...
	/// </summary>
	Vector3 ClipVelocity( Vector3 vel, Vector3 planeVelocity, Vector3 norm, float bounce )
	{
		float planeSpeed = planeVelocity.Dot( -norm );

		float backoff = Vector3.Dot( vel, norm ) * (1f + bounce);
